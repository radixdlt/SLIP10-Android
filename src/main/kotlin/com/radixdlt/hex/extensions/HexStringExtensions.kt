 ...
 */
fun HexString.hexToByteArray(): ByteArray = decode(string)
 ...
 * ```
 */
fun HexString.prepend0xPrefix(): HexString = HexString(if (has0xPrefix()) string else "0x$string")
 ...
 * ```
 */
fun HexString.clean0xPrefix(): HexString = HexString(if (has0xPrefix()) string.substring(2) else string)

/**
 * Returns if a given string is a valid hex-string - either with or without 0x prefix
 */
fun HexString.isValidHex(): Boolean = HEX_REGEX.matches(string)